...
export function MarkdownEditor() {
  ...
  useEffect(() => {
    if (ref.current && !ref.current.dataset.init) {
      ...
      ref.current.innerHTML = INITIAL_NOTES;
    }
  }, []);
...
export type AITab = 'enhanced' | 'summary' | 'actions' | 'decisions';
  ...
  return (
    <div className="ai-card">
      ...
      <div className="ai-content">
        ...
        {tab === 'summary' && (
          <p>
            Discovery call with Dana Chen (Acme, ~40 people). Their team is buried in back-to-back meetings and has
            burned on two prior notetaker tools — one for requiring a visible bot, the other for being Mac-only.
            Lyrola&apos;s no-bot capture and cross-platform support land directly on both objections. Dana flagged
            privacy as a legal review item and asked about cross-meeting search; both covered. She&apos;s open to a
            10-seat pilot pending pricing details.
          </p>
        )}
        {tab === 'actions' && (
          <ul>
